...
function printPriceTable(storeCode, variants) {
  ...
  console.log(
    chalk.bold(
      `  ${storeName} (${storeCode}) - ${inStockCount}/${variants.length} in stock`
    )
  );
...
async function promptCartItems(variants) {
  ...
  console.log(
    chalk.gray("  Enter gram weight, then quantity. Empty gram to finish.\n")
  );
  ...
  while (true) {
    ...
    if (!ref) {
      console.log(
        chalk.red("  Invalid gram. Choose from: " + grams.join(", "))
      );
      continue;
    }
    ...
    if (liveVariant && !liveVariant.inStock) {
      console.log(
        chalk.yellow(`  Warning: ${ref.name} is out of stock at this store.`)
      );
    }
    if (liveVariant) {
...
async function main() {
  ...
  if (useBrowser) {
    ...
    console.log(
      chalk.gray(`  Make sure ${browserName} is running with: ${launchCmd}\n`)
    );
  }
  ...
  const onRateLimit = async (cooldownSec, resumeTime) => {
    ...
    console.log(
      chalk.gray(
        "  Waiting will auto-retry. Restarting clears site data and re-logins."
      )
    );
    const answer = await ask(
      `  Wait and retry, or restart? (w = wait, r = restart): `
    );
  ...
  while (sessionRestart) {
    ...
    try {
        ...
        chalk.cyan(
          "  [2/5] " +
            (useBrowser
              ...
              : "Logging in (with Cloudflare bypass)...")
        )
      );
      ...
      console.log(
        chalk.gray(
          `  Extracted ${cookies.length} cookies, CSRF token: ${
            csrfToken ? "yes" : "no"
          }`
        )
      );
      ...
      if (currentStore && currentStore === storeCode) {
        console.log(
          chalk.green(
            `  [4/5] Already at ${storeName} (${storeCode}), skipping location change`
          )
        );
      } else {
      ...
      let { variants, taxParams } = await api.fetchPricesAndStock(
        useBrowser ? page : null
      );
      ...
      if (inStockCount === 0) {
        ...
        const shouldSwitch = await promptConfirm(
          `Switch to ${otherStoreName} instead?`
        );
        ...
        if (shouldSwitch) {
          ...
          // Re-fetch prices
          const result = await api.fetchPricesAndStock(
            useBrowser ? page : null
          );
          ...
          // Check again
          const newInStockCount = result.variants.filter(
            (v) => v.inStock
          ).length;
      ...
      if (config.raceTabs > 1) {
        ...
        console.log(
          chalk.gray("  You can enter gram + qty while tabs are loading.\n")
        );
        fastPurchasePagePromise = raceLoadPage(
          browser,
          purchaseUrl,
          config.raceTabs,
          {
      ...
      if (fastPurchasePagePromise) {
        try {
          ...
          if (raceWinner && raceWinner !== page) {
            ...
            console.log(
              chalk.gray(
                "  Fastest tab selected. Other racing tabs were closed."
              )
            );
          }
        } catch (err) {
          ...
          console.log(
            chalk.yellow(`  Tab race failed, using current tab: ${err.message}`)
          );
        }
      }
      ...
      while (true) {
        ...
        if (cycle > 1) {
          ...
          try {
            ...
            console.log(
              chalk.red(
                `  Token refresh failed: ${err.message}, retrying cycle...`
              )
            );
        ...
        let cartResult;
        if (useBrowser) {
          cartResult = await api.addToCartViaBrowser(
            activePage,
            itemsWithPrices,
            taxParams
          );
        ...
        if (!cartResult.success) {
          console.log(
            chalk.red(
              `  Add-to-cart failed (HTTP ${cartResult.status}), retrying...`
            )
          );
          continue;
        }
        console.log(
          chalk.green("  Cart updated! (HTTP " + cartResult.status + ")")
        );

        // If redirected to /my-cart, cart has items - skip straight to checkout
        if (cartResult.redirectedTo && cartResult.redirectedTo.includes('/my-cart')) {
          console.log(chalk.gray("  Redirected to cart page - items confirmed, proceeding to checkout"));
        }

        // Close existing tabs - keep a blank tab so browser stays alive
        const allPages = await browser.pages();
        if (allPages.length > 0) {
          // Open a blank tab first, then close the rest
          const blankPage = await browser.newPage();
          for (const p of allPages) {
            if (p !== blankPage && !p.isClosed()) await p.close().catch(() => {});
          }
        }
        ...
        console.log(
          chalk.cyan(
            `  [cycle ${cycle}] Parallel checkout (${config.raceTabs} tabs x ${rounds} rounds)...`
          )
        );
        const results = await runParallelCheckout(
          browser,
          config.raceTabs,
          rounds,
          { onRateLimit, storeCode }
        );

        // Check if user chose to restart due to rate limiting
        const wantsRestart = results.some(
          (r) => r.reason === "rate_limited_restart"
        );
        if (wantsRestart) {
        ...
        const stockOut = results.some((r) => r.reason === "stock_out");

        if (stockOut && storeCode !== 'ABDH') {
          // --- Stock-out recovery: downgrade gram and retry ---
          console.log(chalk.yellow("\n  Stock-out detected! Starting downgrade recovery..."));

          // 1. Open a page and clear cart
          let recoveryPage = await browser.newPage();
          try {
            await recoveryPage.goto(config.BASE_URL + config.endpoints.myCart, {
              waitUntil: 'networkidle2',
              timeout: NAVIGATION_TIMEOUT,
            });

            // Delete all cart items
            const removeCount = await recoveryPage.evaluate(async () => {
              ...
              for (const btn of removeButtons) {
                ...
                const confirmBtn = document.querySelector('.swal-button--confirm, .swal-button--danger');
                if (confirmBtn) confirmBtn.click();
                await new Promise(r => setTimeout(r, 1000));
              }
              return removed;
            });
            console.log(chalk.gray(`  Removed ${removeCount} item(s) from cart`));

            // 2. Navigate to purchase page for fresh stock
            await recoveryPage.goto(config.BASE_URL + config.endpoints.purchasePage, {
              waitUntil: 'networkidle2',
              timeout: NAVIGATION_TIMEOUT,
            });

            // Re-extract tokens
            const freshCookies = await extractCookies(recoveryPage);
            await api.importCookies(freshCookies);
            const freshToken = await extractCsrfToken(recoveryPage);
            if (freshToken) {
              csrfToken = freshToken;
              api.setCsrfToken(freshToken);
            }

            // 3. Fetch fresh stock with availableQty
            const freshData = await api.fetchPricesAndStock(useBrowser ? recoveryPage : null);
            const freshVariants = freshData.variants;
            taxParams = freshData.taxParams;

            console.log(chalk.cyan("  Fresh stock:"));
            for (const v of freshVariants) {
              if (v.inStock) {
                const ref = config.getVariantById(v.id);
                const qtyStr = v.availableQty != null ? ` (qty: ${v.availableQty})` : '';
                console.log(chalk.green(`    ${ref ? ref.gram + 'g' : v.name} - ${formatPrice(v.price)}${qtyStr}`));
              }
            }

            // 4. Calculate original budget and downgrade
            let originalBudget = 0;
            for (const item of itemsWithPrices) {
              originalBudget += (item.price || 0) * item.qty;
            }
            console.log(chalk.gray(`  Original budget: ${formatPrice(originalBudget)}`));

            let remainingBudget = originalBudget;
            const newItems = [];

            for (const item of itemsWithPrices) {
              const currentRef = config.getVariantById(item.variantId);
              if (!currentRef) continue;

              // Find current gram's position in VARIANTS (sorted ascending)
              const currentIdx = config.VARIANTS.findIndex(v => v.id === item.variantId);
              if (currentIdx < 0) continue;

              // Walk down from current gram position to find in-stock alternative
              let found = false;
              for (let i = currentIdx; i >= 0; i--) {
                const candidate = config.VARIANTS[i];
                const liveVariant = freshVariants.find(v => v.id === candidate.id);
                if (!liveVariant || !liveVariant.inStock) continue;

                // Calculate max qty we can afford
                let maxQty = Math.floor(remainingBudget / liveVariant.price);
                if (liveVariant.availableQty != null) {
                  maxQty = Math.min(maxQty, liveVariant.availableQty);
                }
                if (maxQty <= 0) continue;

                const subtotal = liveVariant.price * maxQty;
                ...
                if (candidate.id !== item.variantId) {
                  console.log(chalk.yellow(`  Downgraded: ${currentRef.gram}g -> ${candidate.gram}g x${maxQty} (${formatPrice(subtotal)})`));
                } else {
                  console.log(chalk.green(`  Kept: ${currentRef.gram}g x${maxQty} (${formatPrice(subtotal)})`));
                }
                found = true;
                break;
              }
              ...
              if (!found) {
                console.log(chalk.red(`  No in-stock alternative for ${currentRef.gram}g - skipping`));
              }
            }

            if (newItems.length === 0) {
              console.log(chalk.red("  No items available after downgrade. Retrying cycle..."));
              await recoveryPage.close().catch(() => {});
              continue;
            }

            // 5. Update itemsWithPrices for next add-to-cart
            itemsWithPrices = newItems;
            printCartSummary(itemsWithPrices, freshVariants);

            // 6. Add downgraded items to cart
            console.log(chalk.cyan("  Adding downgraded items to cart..."));
            let downgradeCartResult;
            if (useBrowser) {
              downgradeCartResult = await api.addToCartViaBrowser(recoveryPage, itemsWithPrices, taxParams);
            } else {
              downgradeCartResult = await api.addToCart(itemsWithPrices, taxParams);
            }

            await recoveryPage.close().catch(() => {});

            if (!downgradeCartResult.success) {
              console.log(chalk.red(`  Downgrade add-to-cart failed (HTTP ${downgradeCartResult.status}), retrying cycle...`));
              continue;
            }
            console.log(chalk.green("  Downgraded items added to cart!"));

            // Close existing tabs - keep a blank tab so browser stays alive
            const allPagesForCleanup = await browser.pages();
            if (allPagesForCleanup.length > 0) {
              const blankTab = await browser.newPage();
              for (const p of allPagesForCleanup) {
                if (p !== blankTab && !p.isClosed()) await p.close().catch(() => {});
              }
            }

            console.log(chalk.cyan(`  Re-launching parallel checkout with downgraded items...`));
            const retryResults = await runParallelCheckout(browser, config.raceTabs, rounds, { onRateLimit, storeCode });

            // Process retry results (same as normal)
            const retryRestart = retryResults.some(r => r.reason === 'rate_limited_restart');
            if (retryRestart) throw new Error('RATE_LIMITED_RESTART');

            let retrySuccess = 0;
            for (const r of retryResults) {
              if (r.success) retrySuccess++;
            }
            console.log(chalk.bold(`  Downgrade retry: ${retrySuccess}/${config.raceTabs} tabs completed.`));
            // Continue to next cycle
          } catch (recoveryErr) {
            console.log(chalk.red(`  Stock-out recovery failed: ${recoveryErr.message}`));
            await recoveryPage.close().catch(() => {});
          }
        } else if (cartEmpty) {
          console.log(
            chalk.yellow("  Cart emptied — re-adding items and retrying...")
          );
      ...
      if (err.message === "RATE_LIMITED_RESTART") {
        ...
        console.log(
          chalk.yellow(
            `  The ban is tied to this ${currentBrowserName} instance's TLS fingerprint.`
          )
        );
        console.log(chalk.yellow("  Options:"));
        console.log(
          chalk.yellow(
            "    1) Clear site data and retry (works if ban has expired)"
          )
        );
        console.log(
          chalk.yellow(
            `    2) Exit so you can restart ${currentBrowserName} with a fresh session`
          )
        );
        if (!isFirefox) {
          console.log(
            chalk.yellow(
              "    Tip: Set LM_BROWSER=firefox in .env to use Firefox (different TLS fingerprint)"
            )
          );
        }
        console.log("");
